...
function address() {
	...
	var city=$("#city").val();
	var more_adress=$("#more_adress").val();
	var postalCode=$("#postalCode").val();
	var checkpostalCode=/^[0-9]{6}$/;
	var checkNumber=/^1[3|4|5|8][0-9]\d{4,8}$/;
	if(relName==null||relName==""){
		toastr.warning("姓名不能为空");
		return false;
	}
	
	else if(!checkNumber.test(number)){
			toastr.warning("电话号码不符合要求");
			return false;
	}
	
	else if(more_adress==null||more_adress==""){
		toastr.warning("详细地址不能为空");
		return false;
	}
	else  if(!checkpostalCode.test(postalCode)){
			toastr.warning("邮编不符合要求");
			return false;
		}
	else{
...
function addAddress(){
	document.getElementById("newadd").innerHTML="<input type='submit' class='btn btn-primary' value='添加' >";
	document.getElementById("myModalLabel").innerHTML="添加收货地址";
	for(var i=0;i<7;i++){
	$("#tranform input:eq('"+i+"')").val("");
	}
	$("#tranform").attr("action","addAddress.action");
}
function eidtAddress(addID){
	document.getElementById("newadd").innerHTML="<input type='submit' class='btn btn-primary' value='保存' >";
	document.getElementById("myModalLabel").innerHTML="编辑收货地址";
	$("select option").attr('selected',false);
	$.ajax({url:"findByAddressId.action",
		async:false,
		method:"get",
		dataType:"json",
		data:{formdata:addID},
		success:function(date){
			...
			$.each(json,function(i){
				$("#tranform input[name='"+i+"']").val(json[i]);
				$("#tranform select[name='"+i+"']").prepend("<option value='"+i+"' selected='selected'>"+json[i]+"</option>");
			})
			$("#tranform").attr("action","updateAddress.action");
		},
	    error:function(){
		toastr.warning("编辑失败");
	}
...
function setDefalut(addressx){
	$.ajax({url:"setTheDefaultAddress.action",
		async:false,
		method:"get",
		dataType:"text",
		data:{formdata:addressx},
		success:function(){
		toastr.success("设置成功");
		},
	    error:function(){
